...
import { resolveDishImage, generateNutriIADishVisual, saveCustomDishImage } from '../utils/recipeImages';
import { 
  BookOpen, 
  ChefHat, 
  Clock, 
...
export const RecipesScreen: React.FC = () => {
  ...
  const [isGeneratingModalAiImage, setIsGeneratingModalAiImage] = useState(false);
  ...
  const handleOpenRecipeModal = (recipe: Recipe) => {
    ...
    setModalCustomImage(resolveDishImage(recipe.title, recipe.species, recipe.imageUrl));
    if (selectedPet) {
            ...
            {filteredRecipes.map((recipe) => (
              ...
              >
                ...
                <div className="h-44 relative overflow-hidden bg-gradient-to-br from-[#12241C] to-[#0A1610] flex items-center justify-center">
                  <img 
                    src={resolveDishImage(recipe.title, recipe.species, recipe.imageUrl)} 
                    alt={recipe.title}
      ...
      {activeRecipe && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white dark:bg-[#112019] border border-[#E8DCCB] dark:border-[#D4AF37]/30 rounded-3xl max-w-2xl w-full shadow-2xl p-6 sm:p-8 space-y-6 animate-in fade-in zoom-in-95 my-8 max-h-[90vh] overflow-y-auto">
            ...
            {/* Recipe Dish Visual Presentation Banner */}
            <div className="relative rounded-2xl overflow-hidden h-48 sm:h-60 bg-stone-950 border border-stone-200 dark:border-stone-800 group shadow-md">
              <img 
                src={modalCustomImage || resolveDishImage(activeRecipe.title, activeRecipe.species, activeRecipe.imageUrl)}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="text-white space-y-0.5">
                    <span className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5" />
                      {language === 'es' ? 'Presentación de la Receta' : 'Recipe Dish Presentation'}
                    </span>
                    <p className="text-[11px] text-stone-300">
                      {language === 'es' ? 'Foto creada por Nutri IA o subida desde tu galería' : 'Photo generated by Nutri IA or uploaded from phone'}
                    </p>
                  </div>

                  <div className="flex items-center gap-2">
                    <label className="px-3 py-1.5 rounded-xl bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/25 text-white text-xs font-bold cursor-pointer transition-all flex items-center gap-1.5 shadow-sm active:scale-98">
                      <Upload className="w-3.5 h-3.5" />
                      <span>{language === 'es' ? 'Subir Foto' : 'Upload Photo'}</span>
                      <input 
                        type="file" 
                        accept="image/*" 
                        className="hidden" 
                        onChange={(e) => {
                          ...
                          reader.onload = () => {
                            ...
                            setModalCustomImage(b64);
                            saveCustomDishImage(activeRecipe.title, b64);
                            showToast(language === 'es' ? '📸 ¡Foto del plato guardada con éxito!' : '📸 Dish photo saved!');
                          };
                    ...
                    <button
                      type="button"
                      disabled={isGeneratingModalAiImage}
                      onClick={async () => {
                        setIsGeneratingModalAiImage(true);
                        try {
                          const aiResult = await generateNutriIADishVisual(activeRecipe.title, activeRecipe.species);
                          setModalCustomImage(aiResult.imageUrl);
                          saveCustomDishImage(activeRecipe.title, aiResult.imageUrl);
                          showToast(language === 'es' ? '✨ ¡Nutri IA ha generado la foto de este plato!' : '✨ Nutri IA generated this dish photo!');
                        } catch (err) {
                          console.error(err);
                        } finally {
                          setIsGeneratingModalAiImage(false);
                        }
                      }}
                      className="px-3 py-1.5 rounded-xl bg-[#B8860B] hover:bg-amber-600 text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm active:scale-98 disabled:opacity-50"
                    >
                      {isGeneratingModalAiImage ? (
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                      ) : (
                        <Bot className="w-3.5 h-3.5" />
                      )}
                      <span>{isGeneratingModalAiImage ? (language === 'es' ? 'Creando con IA...' : 'Creating...') : (language === 'es' ? 'Crear con Nutri IA' : 'Generate with Nutri IA')}</span>
                    </button>
                  </div>
                </div>
              </div>
